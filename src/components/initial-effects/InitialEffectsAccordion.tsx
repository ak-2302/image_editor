...
function InitialEffectsAccordion({ values, isOpen, onToggle, onChange }: InitialEffectsAccordionProps) {
  return (
    <div className={`effect_accordion${isOpen ? ' is_open' : ''}`}>
      ...
      {isOpen && <div className="initial_effect_fields">{initialEffectFields.map(({ key, label, min, max, unit }) => <label className="range_control" key={key}><span><span>{label}</span><output>{values[key]}{unit}</output></span><input type="range" min={min} max={max} value={values[key]} onChange={(event) => onChange(key, Number(event.target.value))} /></label>)}</div>}
    </div>
  )
}
