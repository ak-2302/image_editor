...
export default function EffectValueRow({ label, value, min, max, unit, initial, onChange }: Props) {
  ...
  const onPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const move = (e: PointerEvent) => { if (!start.current) return; const delta = e.clientX - start.current.x; if (Math.abs(delta) > 2) dragged.current = true; const next = Math.round(start.current.value + delta / 2); onChange(Math.min(max ?? next, Math.max(min ?? next, next))) }
