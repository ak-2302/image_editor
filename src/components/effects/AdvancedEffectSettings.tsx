...
export default function AdvancedEffectSettings({ effect, onChange }: Props) {
  ...
  switch (effect.name) {
    case "monochrome":
      return <div className="initial_effect_fields"><ColorRow label="色" value={color("monochromeColor", "#ffffff")} onChange={(next) => onChange("monochromeColor", next)} />{row("強度", "monochromeStrength", 100, "%", 0, 100)}</div>;
    case "gradient":
      return <div className="initial_effect_fields"><ColorRow label="開始色" value={color("gradientStartColor", "#ffffff")} onChange={(next) => onChange("gradientStartColor", next)} /><ColorRow label="終了色" value={color("gradientEndColor", "#000000")} onChange={(next) => onChange("gradientEndColor", next)} />{row("角度", "gradientAngle", 0, "°")}{row("強度", "gradientStrength", 0, "%", 0, 100)}</div>;
    case "glow":
    ...
    case "mosaic":
      return <div className="initial_effect_fields">{row("サイズ", "mosaicSize", 8, "px", 1)}{row("強度", "mosaicStrength", 0, "%", 0, 100)}</div>;
    case "clipping":
    ...
    case "diagonalClipping":
      return <div className="initial_effect_fields">{row("角度", "diagonalClipAngle", 45, "°")}{row("位置", "clipTop", 0, "px", 0)}{row("ぼかし", "maskBlur", 0, "px", 0)}<label className="initial_effect_row"><span>反転</span><input type="checkbox" checked={Boolean(effect.values.maskInvert)} onChange={(event) => onChange("maskInvert", event.target.checked)} /></label></div>;
    case "mask":
      return <div className="initial_effect_fields"><label className="initial_effect_row"><span>形状</span><select value={effect.values.maskShape ?? "rectangle"} onChange={(event) => onChange("maskShape", event.target.value as EffectParameters["maskShape"])}><option value="rectangle">四角</option><option value="circle">円</option><option value="ellipse">楕円</option></select></label>{row("X", "maskX", 0, "px")}{row("Y", "maskY", 0, "px")}{row("幅", "maskWidth", 100, "px", 0)}{row("高さ", "maskHeight", 100, "px", 0)}{row("ぼかし", "maskBlur", 0, "px", 0)}<label className="initial_effect_row"><span>反転</span><input type="checkbox" checked={Boolean(effect.values.maskInvert)} onChange={(event) => onChange("maskInvert", event.target.checked)} /></label></div>;
    case "shadow":
      return <div className="initial_effect_fields"><ColorRow label="色" value={color("shadowColor", "#000000")} onChange={(next) => onChange("shadowColor", next)} />{row("濃さ", "shadowOpacity", 0, "%", 0, 100)}{row("距離", "shadowDistance", 8, "px", 0)}{row("角度", "shadowAngle", 45, "°")}{row("ぼかし", "shadowBlur", 8, "px", 0)}</div>;
    case "outline":
      return <div className="initial_effect_fields"><ColorRow label="色" value={color("outlineColor", "#000000")} onChange={(next) => onChange("outlineColor", next)} />{row("太さ", "outlineWidth", 0, "px", 0)}{row("濃さ", "outlineOpacity", 100, "%", 0, 100)}</div>;
    case "imageLoop":
